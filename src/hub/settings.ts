/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 GitHub Token 配置
 */
...
/**
 * POST /settings/save — 保存修改后的 GitHub Token 配置
 */
...
function renderConfigForm(
  ...
  const maskedToken = maskValue(cfg.github_token || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>GitHub — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>GitHub 设置</h1>
    <p class="desc">修改您的 GitHub Personal Access Token 配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 Token</dt>
      <dd>${maskedToken || "（未配置）"}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="github_token" type="password" placeholder="留空则不修改" />
      <p class="hint">以 ghp_ 开头，在 <a href="https://github.com/settings/tokens" target="_blank">GitHub Settings</a> 创建</p>
